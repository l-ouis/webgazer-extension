...
async function loop() {
  if (!paused) {
    ...
    // EXTENSION:
    // needs: ridgereg screen trail clicks array from content script.
    ...
    // EXTENSION: put this in popup or options or smth
...
var clickListener = async function(event) {
  ...
  // EXTENSION: listen via chrome messages from content script
...
var moveListener = function(event) {
  ...
  // EXTENSION: also listen thru messages.
...
var addMouseEventListeners = function() {
  // EXTENSION: we need this from content script, everything else can be done inside background
...
var removeMouseEventListeners = function() {
  ...
  chrome.runtime.onMessage.removeListener(moveListener);
};
